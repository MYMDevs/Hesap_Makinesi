...
import com.example.ythesapmakinesi.databinding.ActivityMainBinding;
...
public class MainActivity extends AppCompatActivity {
    ...
    private boolean noktaSorgulama() {
        ...
        char lastChar = islemler.charAt(islemler.length() - 1); // Girişin son karakterini kontrol eder.

        if (Character.isDigit(lastChar)) { // Eğer son karakter rakamsa, nokta eklenebilir mi diye kontrol eder.
            String lastNumber = islemler.split("[+\\-x÷]")[islemler.split("[+\\-x÷]").length - 1]; // Son sayıyı alır.
            return !lastNumber.contains("."); // Eğer sayıda zaten nokta yoksa, nokta eklenebilir.
        }

        return false; // Eğer son karakter operatörse, nokta eklenemez.
    }

    private String sifirDuzenleme(String islemler, String yeniRakam) {
        if (islemler.matches(".*[+\\-x÷]0$")) { // İşlem sonunda 0 varsa, bunu yeni rakam ile değiştirir.
            islemler = islemler.substring(0, islemler.length() - 1) + yeniRakam;
        } else {
            islemler += yeniRakam; // Aksi takdirde, yeni rakamı ekler.
        }
        return islemler;
    }
    ...
    private boolean OperatorUygunluguSorgulama() {
        ...
        char lastChar = islemler.charAt(islemler.length() - 1); // Girişin son karakterini kontrol eder.
        return Character.isDigit(lastChar); // Eğer son karakter rakamsa, operatör eklenebilir.
    }
